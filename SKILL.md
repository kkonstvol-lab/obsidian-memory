...
description: "Build and maintain a persistent knowledge + memory system in Obsidian for AI agents. The skill combines three interlocking systems: (1) LLM Wiki — a curated knowledge base of summaries, entities, and concepts extracted from books, articles, and PDFs, organized with wikilinks and Maps of Content; (2) Robby Palace — a MemPalace-inspired layer of Wings (person/project profiles) and Drawers (immutable session logs) that compile lived experience into structured context; (3) Self-Improvement Loop — per-agent private memory (active focus, corrections, backlog, heartbeat, metrics) that learns from mistakes without agents contaminating each other's context. On top of all three sits a Graphify knowledge graph that auto-detects semantic connections between wiki pages using Leiden clustering and exposes them via MCP tools, so agents query the graph before grep. Supports multi-agent setups (e.g. Claude Code + Codex) where world-level facts are shared and operational memory is fully isolated per agent. Invoke for: SETUP, INGEST, QUERY, LINT, GRAPH, MEMORY, COMPILE, WING. Triggers: 'ingest', 'add to wiki', 'query wiki', 'lint wiki', 'obsidian memory', 'obsidian wiki', '/obsidian-memory', 'llm wiki', 'knowledge base', 'wiki setup', 'compile', 'create wing', 'person profile', 'project profile', 'knowledge graph', 'graphify'."
---
