...
OPERATIONS: tuple[Operation, ...] = (
    ...
    Operation("release-status", "Build a read-only release/push packet with repo, branch, surface, risks, checks, and post-push obligations.", "read_only", False, ["git", "12-{agent}", "12-shared", "wiki"], "advisory_report", ["python3 assets/operator/release_status.py --intent public-obsidian-memory"]),
    Operation("branch-close", "Build a read-only branch close handoff with repo state, memory context, backlog, risks, and verification commands.", "read_only", False, ["git", "12-{agent}", "12-shared/graph"], "advisory_report", ["python3 assets/operator/branch_close_pack.py --md"]),
    Operation("lesson-review", "Render a review board for private agent operational lessons before promotion or activation.", "read_only", False, ["12-{agent}/lessons"], "advisory_report", ["python3 assets/operator/lesson_review_board.py --md"]),
    Operation("release-surface-check", "Check selected release surface markers, remotes, dirty state, and public readiness without external calls.", "read_only", False, ["git", "12-shared", "public repos"], "advisory_report", ["python3 assets/operator/release_surface_check.py --profile public-skill --md"]),
    Operation("decision-review", "Render a read-only board of decisions that may need durable routing into memory, wiki, lessons, runbooks, or public surfaces.", "read_only", False, ["git", "12-{agent}", "12-shared", "wiki"], "advisory_report", ["python3 assets/operator/decision_review_board.py --md"]),
    Operation("decision-review-mark", "Append an approved decision-review lifecycle record for a candidate id.", "mutating", True, ["12-{agent}/decision-review"], "review_state", ["MEMORY_OPERATOR_APPROVED=1 python3 assets/operator/decision_review_board.py --mark <id> --status skipped --note '<reason>'"]),
